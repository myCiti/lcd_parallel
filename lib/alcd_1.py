...
class LCD:  # LCD objects appear as read/write lists
    ...
    def move_to(self, cursor_x, cursor_y):
        ...
        if cursor_y & 2:    # Lines 2 & 3 add number of columns
            addr += 2
        self.lcd_byte(0x80 | addr, LCD.CMD)

    def write_line(self, msg, line):
        self.lcd_byte(LCD.LCD_LINES[line], LCD.CMD)
        for byte in msg:
            self.lcd_byte(ord(byte), LCD.CHR)

    def write_line2(self, msg, line, pos):
        cursor_x = pos - 1 
        cursor_y = line - 1
...
def test1():
    ...
    PINLIST = [16, 17, 18, 19, 20, 21, 15, 14, 13, 12]
